...
- Rho case-study popup source of truth: Figma node `851:14271` (`Rho Main`). Open it from the Rho project card as a full-screen modal and preserve its Inter typography, 40px desktop margins, centered 596px TLDR column, and original Figma imagery. The current hero node `851:14632` is 672 × 606 CSS px and should use a whole-image 4× export at 2688 × 2424 px. The project metadata order is Role → Time → Scope.
- Loomy case-study popup source of truth: Figma node `859:14918` (`Loomy Main`). Open it from the Loomy project card as a full-screen modal. Its logo, 672 × 606 hero, and three phone compositions use source-exported imagery. Per the later user request, Home Screen additionally layers a source-exported orange device-frame PNG above its full-node export. The current metadata includes the Motion tag and shows Role → Time → the official App Store badge; Rate is no longer shown. The badge links to `https://apps.apple.com/gb/app/loomy-fonts-captions-stickers/id1512686487`.
